...
const SECTORS = [
  ...
  { icon: Factory, label: 'Manufacturing', sub: 'Coming soon', active: false },
  { icon: FlaskConical, label: 'Pharma', sub: 'Coming soon', active: false },
  { icon: Package, label: 'Food & Beverage', sub: 'Coming soon', active: false },
];
    ...
    <div className="min-h-screen bg-[#0f1117] dot-grid text-slate-200">
      ...
      <header className="border-b border-[#2a3348] bg-[#0f1117]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-lg font-bold tracking-tight text-white">STOCHOPT</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-20 space-y-28">
        ...
        <section className="text-center animate-slide-up">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border border-sky-500/30 bg-sky-500/10 text-sky-400 mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" />
            Stochastic Linear Programming Engine
          </span>

          <h1 className="text-5xl sm:text-6xl font-extrabold text-white tracking-tight leading-tight mb-6">
            Production Planning<br />
            <span className="text-sky-400">Under Uncertainty</span>
          </h1>

          <p className="text-lg text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
            A two-stage stochastic optimization platform that lets you plan for every scenario,
            maximize expected profit, and make decisions that hold up in the real world.
          </p>
        ...
        <section ref={sectorsRef}>
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400">Agriculture is fully operational. More sectors launching soon.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {SECTORS.map(({ icon: Icon, label, sub, active }) => (
              <button
                ...
                disabled={!active}
                className={`card text-left group transition-all duration-300 ${active
              ...
              >
                ...
                {active && (
                  <div className="mt-4 flex items-center gap-1 text-xs text-sky-400 font-semibold">
      ...
      <footer className="border-t border-[#2a3348] mt-10 py-8">
        <p className="text-center text-xs text-slate-600 font-mono">
          STOCHOPT · Two-Stage Stochastic Production Planning · API at{' '}
